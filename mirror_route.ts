...
export async function handleMirrorRoute(context: Context) {
    ...
    if (!range) {
        // give the full video if no range is specified
        ...
        if (videoRequest.ok) {
            ...
            context.response.headers.set("content-length", videoRequest.headers.get("content-length") || "");
            context.response.headers.set("accept-ranges", "bytes");
            context.response.body = readableStream;
        }
    } else {
        // give the video in chunks if range is specified
        const videoRequest = await fetch(videoUrl, {
            headers: {
                "Range": range,
            },
