...
export async function getVideoUrl(videoId: number | null, address: string | null) {
    ...
    const videoProviderRequest = await fetch(`https://www.animeunity.so/embed-url/${videoId}` , {
        "headers": {
    ...
    //     let preprocessed = videoPlaylist[1]
    //         .replace(/(\w+):\s/g, '"$1":')        // Quote object keys
    //         .replace(/'/g, '"')                // Replace single quotes with double quotes
    //         .replace(/,\s*([}\]])/g, '$1')
    //         .replace(/\n/g, '');
    //     const videoPlaylistJson = JSON.parse(preprocessed);
